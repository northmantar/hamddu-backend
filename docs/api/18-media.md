...
| 백엔드 압축 | ❌ 없음 (파일 그대로 R2 업로드) | ✅ sharp로 리사이즈 + 재인코딩 (**원본 포맷 유지**) |
| 압축 정책 | — | 긴 변 ≤ 1200px, 품질 75. 포맷은 원본 유지 |
...
>
> **포맷 보존:** 압축은 원본 포맷을 유지합니다 — JPEG→JPEG, PNG→PNG(투명도 유지), WebP→WebP. **SVG·GIF·AVIF 등**은 재인코딩하면 벡터/애니메이션이 깨지므로 **리사이즈 없이 원본 그대로** 업로드합니다(확장자·MIME 보존).
