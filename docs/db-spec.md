...
### 포인트 액션 타입 lookup / 보상 카탈로그 (`point_action_types`)

| name | type | description |
| --- | --- | --- |
| code | varchar(50) | `<<pkey>>` 액션 코드 (예: `WATCH`, `BOARD_CREATE`) |
| label_ko | varchar(100) | 한글 라벨 (예: `시청`, `게시글 작성`) |
| ref_type | varchar(50) | 보상 이벤트가 발생하는 참조 테이블명 (예: `board`) |
| ref_action | reward_action_enum | CRUD 액션 (`CREATE`/`READ`/`UPDATE`/`DELETE`) |
| is_active | boolean | 활성화 여부 |
| created_at | timestamp | 생성 일시 |
| updated_at | timestamp | 수정 일시 |
- `UNIQUE(ref_type, ref_action)` — 이벤트당 카탈로그 1행.
- 보상 정책 v2(데이터 주도): 큐 프로세서가 발생 이벤트 `(ref_type, ref_action)`로 카탈로그→정책을 조회해 자동 지급. (ref/reward-policy-v2.md)
- 어드민에서 런타임 추가/수정/삭제 가능. **단, `(ref_type, ref_action)`은 코드 레지스트리(`REWARD_EVENTS`, `GET /points/reward-events`)에 등록된 쌍만 생성 허용.**
- `ref_type`은 물리 테이블명이 원칙이나, 도메인 이벤트는 논리 이벤트명 사용 가능 (예: `tutorial_watch` = 튜토리얼 시청 완료, refId=watch_history.id).
- 시드: `WATCH`(tutorial_watch,CREATE, "튜토리얼 시청 완료"), `CHALLENGE`(challenge,CREATE), `COMMENT`(board_comment,CREATE), `BOARD_CREATE`(board,CREATE), `USER_SIGNUP`(users,CREATE).
- 마이그레이션: `1750400000000`(ref 컬럼+BOARD_CREATE), `1750500000000`(USER_SIGNUP), `1750600000000`(WATCH를 tutorial_watch로 리타깃).
- 기존 `point_action_type_enum`은 1750100000000에서 제거, `ref_type`/`ref_action`은 1750400000000에서 추가.
...
### XP 액션 타입 lookup / 보상 카탈로그 (`xp_action_types`)

| name | type | description |
| --- | --- | --- |
| code | varchar(50) | `<<pkey>>` 액션 코드 (예: `USER_SIGNUP`, `WATCH`) |
| label_ko | varchar(100) | 한글 라벨 |
| ref_type | varchar(50) | 보상 이벤트 참조 테이블/논리 이벤트명 |
| ref_action | reward_action_enum | CRUD 액션 |
| is_active | boolean | 활성화 여부 |
| created_at | timestamp | 생성 일시 |
| updated_at | timestamp | 수정 일시 |
- 포인트 카탈로그와 **독립 관리**(별도 code/금액)하되, `(ref_type, ref_action)`은 **동일한 공유 레지스트리**(`REWARD_EVENTS`, 같은 emit이 두 큐로 fan-out)에서만 생성 허용. `UNIQUE(ref_type, ref_action)`.
- 시드: `USER_SIGNUP`(users), `WATCH`(tutorial_watch), `CHALLENGE`(challenge), `COMMENT`(board_comment), `BOARD_CREATE`(board) — 전부 CREATE.
- 1750200000000 신설, 1750700000000에서 ref 컬럼 추가 + 카탈로그 교체.
...
| is_one_time | boolean | 최초 1회(유저 평생) 적립 여부 |
| is_active | boolean | 정책 유효여부 |
| created_at | timestamp | 생성 일시 |
| updated_at | timestamp | 수정 일시 |
