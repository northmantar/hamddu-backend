...
입력한 닉네임이 이미 사용 중인 경우 서버가 숫자 접미사를 붙여 고유한 닉네임으로 자동 변경합니다. 409를 반환하지 않습니다.
...
**응답 `200`:** [유저 객체](#유저-객체) (실제 적용된 닉네임이 `nickname` 필드에 반영됨)
...
```
GET /api/nicknames/check?value={nickname}
```
...
| 필드 | 타입 | 설명 |
|---|---|---|
| `isTaken` | boolean | `true`이면 이미 사용 중인 닉네임 |

---

### 12. 닉네임 자동 발급

서버가 랜덤 닉네임을 생성하여 반환합니다. 발급된 닉네임은 즉시 `users` 테이블에 등록됩니다.

```
POST /api/nicknames/issue
```
...
| 필드 | 타입 | 설명 |
|---|---|---|
| `nickname` | string | 발급된 닉네임. 중복 시 숫자 접미사가 붙습니다 (예: `포근한 실뭉치2`) |

---

### 13. 닉네임 직접 등록

유저가 직접 입력한 닉네임을 등록합니다. 중복 시 서버가 숫자 접미사를 붙여 고유한 닉네임을 반환합니다.
...
| 필드 | 타입 | 설명 |
|---|---|---|
| `nickname` | string | 최종 등록된 닉네임. 충돌 시 접미사가 붙어 반환됩니다 (예: `실뭉치장인2`) |

---

### 14. 닉네임 후보 목록 조회

시퀀스를 소비하지 않고 랜덤 닉네임 후보를 반환합니다. 후보는 예약되지 않으며, 실제 등록은 `POST /nicknames/register`를 통해 이루어집니다.

```
GET /api/nicknames/candidates?count=10
```
...
| 파라미터 | 타입 | 필수 | 기본값 | 설명 |
|---|---|---|---|---|
| `count` | number | No | `10` | 반환할 후보 수. 최대 `20` |

**응답 `200`:**

```json
["포근한 실뭉치", "따뜻한 바늘", "신비로운 코바늘"]
```

중복 없는 `count`개의 닉네임 문자열 배열을 반환합니다.

---

## 닉네임 자동 발급 알고리즘

`POST /api/nicknames/issue` 호출 시 아래 절차로 닉네임이 생성됩니다.

### 기본 닉네임 생성 (형용사 + 명사 조합)

1. DB에서 활성화(`isActive: true`)된 형용사 목록(`m`개)과 명사 목록(`n`개)을 조회합니다.
2. PostgreSQL 시퀀스 `nickname_combo_seq`에서 전역 순번(`seq`)을 원자적으로 발급받습니다.
3. 서로소(coprime) 스텝 순회 방식으로 조합 인덱스를 계산합니다.
   - 전체 조합 수: `total = m × n`
   - 스텝(`step`): `total`과 서로소인 수 (시드 97 기준으로 선택)
   - 인덱스: `index = (17 + seq × step) % total`
   - 이 방식은 시퀀스가 증가할 때마다 전체 조합을 중복 없이 순회합니다.
4. 인덱스를 (형용사, 명사) 쌍으로 변환합니다.
   - `adjectiveIndex = ⌊index / n⌋`
   - `nounIndex = index % n`
5. 최종 기본 닉네임: `"형용사 명사"` (예: `"포근한 실뭉치"`)

### 중복 처리 (접미사 부여)

기본 닉네임을 `users` 테이블에 삽입(`ON CONFLICT DO NOTHING`)하여 등록을 시도합니다.

- **성공 시:** 해당 닉네임을 그대로 반환합니다.
- **충돌 시:** `nickname_bases` 테이블에서 해당 닉네임의 `next_suffix`를 원자적으로 1 증가시키고 이전 값을 접미사로 사용합니다 (초기값 2, 이후 3, 4, … 순서로 증가). 고유한 닉네임이 확보될 때까지 반복합니다.
