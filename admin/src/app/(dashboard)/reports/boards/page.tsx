...
import { useBoardReports, useUpdateBoardReport } from '@/hooks/queries/use-reports';
import { useToast } from '@/components/ui/toast';
import type { BoardReport, ReportStatus } from '@/types';
...
export default function BoardReportsPage() {
  ...
  const updateReport = useUpdateBoardReport();
  const { addToast } = useToast();

  const handleProcess = async (report: BoardReport, status: 'resolved' | 'rejected') => {
    const label = status === 'resolved' ? '미노출 처리' : '기각';
    if (!confirm(`이 신고를 "${label}"하시겠습니까?${status === 'resolved' ? '\n해당 게시글이 숨김 처리됩니다.' : ''}`)) return;
    try {
        ...
        {isLoading ? (
          ...
          <table className="w-full">
            <thead className="bg-gray-50 text-xs text-gray-500 uppercase">
              <tr>
                <th className="px-4 py-3 text-left">게시글</th>
              ...
              {data.data.map((report) => (
                <tr key={report.id} className="border-t hover:bg-gray-50">
                  <td className="px-4 py-3">
                    <div className="text-sm font-medium max-w-xs truncate">{report.board.title}</div>
                    <div className="text-xs text-gray-400">{report.board.id.slice(0, 8)}...</div>
                  </td>
